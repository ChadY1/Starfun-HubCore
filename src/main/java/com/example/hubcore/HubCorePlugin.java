...
public class HubCorePlugin extends JavaPlugin {
    ...
    public void reloadStarfun() {
        ...
        String channel = getConfig().getString("bungeecord.channel", "BungeeCord");
    ...
    private void initializeSystems() {
        ...
        if (getConfig().getBoolean("bungeecord.enabled", true)) {
            String channel = getConfig().getString("bungeecord.channel", "BungeeCord");
            getServer().getMessenger().registerOutgoingPluginChannel(this, channel);
        } else {
            getLogger().info("Bungeecord messaging disabled via config.bungeecord.enabled=false");
        }
    ...
    private boolean isValidKey(String key) {
        return key != null && !key.trim().isEmpty() && !key.equalsIgnoreCase("CHANGE_ME_GENERATED_BASE64_AES_KEY");
    }
